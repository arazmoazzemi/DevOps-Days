...
#### GETSET(Set(Change) vlaue For latest record)
```bash
getset x test
```

#### PERSIST(Remove the existing timeout on key, turning the key from volatile to persistent (a key that will never expire as no timeout is associated)
```bash
